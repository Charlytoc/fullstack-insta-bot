    ...
    for entry in payload.get("entry", []):
        for messaging_event in entry.get("messaging", []):
            ...
            if message and sender_id:
                ...
                conversation = supabase.table("conversations").select("*").eq("sender_id", sender_id).eq("recipient_id", recipient_id).execute()
                if not conversation.data:
                    # Create new conversation
                    conversation = supabase.table("conversations").insert({"sender_id": sender_id, "recipient_id": recipient_id, "title": ""}).execute()
                    conversation_id = conversation.data[0]["id"]
                else:
                    conversation_id = conversation.data[0]["id"]

                # Retrieve last 10 messages
                messages = supabase.table("messages").select("text, type").eq("conversation_id", conversation_id).order("created_at", desc=True).limit(10).execute()
                context = "\n".join([f"{msg['type']}: {msg['text']}" for msg in messages.data])

                # Save incoming message
                supabase.table("messages").insert({"conversation_id": conversation_id, "type": "user", "text": message, "raw_text": message}).execute()
                ...
                # Save bot response
                supabase.table("messages").insert({"conversation_id": conversation_id, "type": "bot", "text": bot_response, "raw_text": f"{system_prompt}\nUser: {message}"}).execute()
